...
def draw(img, cascade, scaleFactor, minNeighbors, color, clf):
    ...
    for x,y,w,h in features:
        ...
        if con <= treshold:
            ...
            img[qr_y_offset:qr_y_offset+qrcode_pic.shape[0], qr_x_offset:qr_x_offset+qrcode_pic.shape[1]] = qrcode_pic
            img[gate_y_offset:gate_y_offset+gate_pic.shape[0], gate_x_offset:gate_x_offset+gate_pic.shape[1]] = gate_pic
...
def usage():
    print("""How to use....
                ...
                [Q]uit          : Enter Q to Quit the program""")
def main():
    global id
    global face
    try:
        ...
        pic_no = 0
        while True:
            choice = input("[U]sage, [D]ata, [I]nsertData To DB, [C]reateDataset, [T]rainDataset, [S]tart, [Q]uit : ")
            if choice in ["U","u"]:
                usage()
            elif choice in ["d","D"]:
                index = len(_fname)

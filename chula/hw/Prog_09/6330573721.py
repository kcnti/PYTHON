...
def approx_match(stations, name):
    return [i for i in stations if ''.join(name.upper().split()) in i]

def top_k_min_temp_stations(stations, K):
    temp = list({key: value for key, value in sorted(stations.items(), key=lambda item: item[1]['temp'])})
    return temp[:K]

def top_k_max_temp_stations(stations, K):
    temp = list({key: value for key, value in sorted(stations.items(), key=lambda item: item[1]['temp'])})
    return temp[len(stations)-K:]

def peak_stations(stations):
    lat = list({key: value for key, value in sorted(stations.items(), key=lambda item: item[1]['lat'])})
    _long = list({key: value for key, value in sorted(stations.items(), key=lambda item: item[1]['long'])})
    return [lat[0], lat[-1], _long[0], _long[-1]]

def k_nearby_stations(stations, main_station, K):
    main_station = ' '.join([i.upper() for i in main_station.split()])
    lat1, long1 = stations[main_station]['lat'], stations[main_station]['long']
    nearest = [[distance(lat1, long1, stations[i]['lat'], stations[i]['long']), i] for i in stations].sort()
    return nearest[1][:K+1]


def average_temp(stations, names):
    temp = [stations[i]['temp'] for i in stations if i in names]
